...
The main result is not that one algorithm "wins." The useful result is that longitudinal telemetry materially improves retrieval of rare failures, and that the intervention threshold changes the business outcome.

At a 25% intervention threshold in the synthetic holdout:

- 500 of 640 impending failures are detected
- 200 healthy terminals are flagged
...
1. **Intervention threshold** — move the threshold and watch failures detected, healthy terminals flagged, intervention volume and estimated support impact update.
